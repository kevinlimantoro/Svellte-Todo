	  ...
	  <div class="column is-narrow" style="width: 70%">
		...
		<ul class:list={todos.length > 0}>
		  {#each todos as todo (todo.id)}
			<li class="list-item" transition:slide="{{duration: 200, easing: elasticInOut}}">
		  ...
		  {:else}
			<li class="has-text-centered" transition:slide="{{duration: 50, easing: elasticInOut}}">Nothing here!</li>
		  {/each}
		</ul>
	  </div>
	</div>
